...
private:
	uint8_t     nametbl[2][1024];
	uint8_t     patterntbl[2][4096];
	uint8_t		palettetbl[32];
...
public:
	...
	uint8_t cRead(uint16_t addr, bool rdonly = false);
	void    cWrite(uint16_t addr, uint8_t  data);

	// Communications with PPU Bus
	uint8_t pRead(uint16_t addr, bool rdonly = false);
	void    pWrite(uint16_t addr, uint8_t data);
